...
type Snapshot struct {
	config   *params.ZephyriaConfig // Consensus engine parameters to fine tune behavior
...
func loadSnapshot(config *params.ZephyriaConfig, sigCache *lru.ARCCache, db ethdb.Database, hash common.Hash, ethAPI *ethapi.BlockChainAPI) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("zephyria-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("zephyria-"), s.Hash[:]...), blob)
}
...
// GetSafeBlock determina si el siguiente bloque es seguro para un validador
// Devuelve el número del bloque que se considera seguro para firmar por el validador.
func (s *Snapshot) GetSafeBlock(header *types.Header, validator common.Address) uint64 {
	// Verifica si el validador está a una distancia segura del último bloque
	if !s.enoughDistance(validator, header) {
		return 0 // No está a una distancia segura, por lo que no se considera seguro firmar el siguiente bloque
	}

	// Verifica si el validador es el próximo en la rotación
	if s.inturn(validator) {
		return 0 // Es el turno del validador, por lo que no se considera seguro firmar el siguiente bloque
	}

	// Verifica si el validador ha firmado recientemente
	if s.SignRecently(validator) {
		return 0 // El validador ha firmado recientemente, por lo que no se considera seguro firmar el siguiente bloque
	}

	// Si pasa todas las verificaciones anteriores, se considera seguro firmar el siguiente bloque,
	// por lo que se devuelve el número del bloque siguiente al último bloque.
	return s.Number + 1
}

func (s *Snapshot) IsLastBlockInSnapshot(lastBlockHash common.Hash) bool {
	return s.Hash == lastBlockHash
}
