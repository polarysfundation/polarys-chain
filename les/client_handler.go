...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	forkid := forkid.NewID(h.backend.blockchain.Config(), h.backend.BlockChain().Genesis().Hash(), h.backend.blockchain.CurrentHeader().Number.Uint64(), h.backend.blockchain.CurrentHeader().Time)
